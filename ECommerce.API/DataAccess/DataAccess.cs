...
namespace ECommerce.API.DataAccess
{
    public class DataAccess : IDataAccess
    {
        ...
        public DataAccess(IConfiguration configuration)
        {
            ...
            dbconnection = this.configuration.GetConnectionString("ShoppingDB");
        ...
        public List<Product> GetProducts(string category, string subcategory, int count)
        {
            ...
            using (SqlConnection connection = new SqlConnection(dbconnection))
            {
                ...
                while (reader.Read())
                {
                    ...
                    var categoryid = (int)reader["CategoryId"];
                    products.ProductCategory = GetProductCategory(categoryid);

                    var offerid = (int)reader["OfferId"];
                    products.Offer = GetOffer(offerid);
        ...
        public string LoginUser(string email, string password)
        {
            ...
            using (SqlConnection connection = new SqlConnection(dbconnection))
            {
                ...
                string key = "t3RVO7GnWzGo4J5YbSXpFJ0uWRHKNeGYM6wTkZkYeVE";
                string duration = "60";
                var symmetrickey = new SymmetricSecurityKey(Encoding.UTF8.GetBytes(key)); //Encode key
                var credentials = new SigningCredentials(symmetrickey, SecurityAlgorithms.HmacSha256);// algorithm used to encode key
